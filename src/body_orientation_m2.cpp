 ...
 *  
 *  first histogram filter and then floodflow 
 *
 * */
...
int histogramFilter(void** pts, vector<int>& indices, vector<bool>& bs);
int histogramFilterFull(void** pts, int N, vector<bool>& bs); 
...
{	
	...
	vector<int> indices; 
        ...
        // 1. first histogram filter 
        // histogramFilterFull((void**)(&points), 640*480, b_remained); 

	// if(body_extract.segmentFromCentral((void**)(&points), depth_intrin.width, depth_intrin.height, indices))
        if(histogramFilterFull((void**)(&points), 640*480, b_remained) >= 30000)
	{
                // histogramFilter((void**)(&points), indices, b_remained); 
            
              if(body_extract.segmentFromCentralWithFlag((void**)(&points), depth_intrin.width, depth_intrin.height, indices, b_remained))
              { 
                ...
                for(int i=0; i< indices.size(); i++)
		{
			...
			glVertex(*pt);
                ...
                cout<<" impossible! something mistake here !"<<endl;
              }
	}
	else
	{
...
int histogramFilterFull(void** pts, int N, vector<bool>& bs)
{
  ...
  return histogramFilter(pts, indices, bs); 
}
...
int histogramFilter(void** pts, vector<int>& indices, vector<bool>& bs)
{
  ...
  for(int i=0; i<N; i++)
  {
    ...
    mx = x2map(pt->x);
    mz = z2map(pt->z); 
    // if(mx < 0 || mx >= MAX_X || mz <0.4 || mz >= MAX_Z) 
    if(pt->x < -1.5 || pt->x > 1.5 || pt->z < 0.4 || pt->z >=1.5 || mx < 0 || mx >= MAX_X || mz <0 || mz >= MAX_Z) 
    {
      continue ;
    }
...
*/
  ...
  for(int i=0; i<N; i++)
  {
    ...
    int mx = x2map(pt->x);
    int mz = z2map(pt->z); 
    if(pt->x < -1.5 || pt->x > 1.5 || pt->z < 0.4 || pt->z >=1.5 || 
        mx < 0 || mx >= MAX_X || mz <0 || mz >= MAX_Z) 
    {
      ++out_range_cnt;
      continue ;
    }
